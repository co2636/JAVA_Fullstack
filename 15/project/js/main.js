...
$(function(){
  ...
  */
   ...
   */
    ...
    /*
      1. index.html을 웹브라우저로 처음 요청 했을때 첫 화면은 [1] 버튼이미지에 관한 배너1 이미지만 보이게 하고
   ...
   var onBtn = $("#roll_banner_wrap dt a:first");
    ...
    /* 
      2. [1]~[4] <img>태그들을 감싸고 있는 모든 <a>들을 선택해서 click 이벤트를 등록한다.

      [1]을 제외한 [2] 또는 [3] 또는 [4] <a>를 클릭하면 [1] 이미지는 하얀색 이미지로 변경해야하고
      클릭한 숫자의 이미지는 빨간색으로 변경 시켜야함
    */
    $("#roll_banner_wrap dt a").on("click", function(){
      ...
      //onBtn변수에 저장되어 있는 <a>의 하위요소 <img>를 선택해 src속성의 이미지 주소값을 하얀색 [1] 이미지 주소로 변경
      // 참고. attr("속성", "새 주소 값");
      $("img", onBtn).attr("src", 
                                  $("img", onBtn)
                                  .attr("src")
      ...
      //[1] ~ [4] <a> 중 하나를 클릭했을때 클릭한 <a>태그를 선택해 <a>의 index번호를 변수에 저장
      var num = $("#roll_banner_wrap dt a").index(this);
      
      //클릭한 <a>의 index 위치 번호값과 일치하는 <dd>태그영역(배너이미지)만 나타나게 해야한다.
      // 자세히 : 변수 num에 저장된 클릭한 <a>의 index위치 번호 값을 이용해 그에 해당하는 배너를 포함하는 <dd>태그를 
      //        선택해서 show()메소드로 보여줌
      ...
      //[1] ~ [4] 버튼 중 click 이벤트가 발생한 <a>요소를 선택해서 onBtn변수에 저장
      onBtn = $(this);
    ...
    */
    ...
    //autoPlay함수 정의 
    //  : 4초 간격으로 호출되어 [1] ~ [4]가 순차적으로 강제로 클릭되어 자동으로 배너이미지영역이 보이도록 하는 기능 정의
    
    //autoPlay 함수 코드 흐름
    // 최초 한번은 3초 쉬었다가 autoPlay함수 호출하여 실행문을 실행시키고 그 다음 2번째 부터는 4초 간격으로 반복하여 
    // 자기 자신의 함수 autoPlay함수를 호출(재귀함수 호출)하여 실행문을 반복 실행 시킵니다. 
    var btnNum = 0;
    function autoPlay(){// [1] ~ [4] 버튼이 순서대로 강제 클릭되면 호출되는 함수

      //일정한 시간간격으로 실행문이 실행될때마다 변수 btnNum의 값이 1씩 증가 되게 하자
      btnNum++;

      // 이때 btnNum 변수의 값이 4이상되는 순간 0으로 변경해서 [1] <a>를 강제 클릭할 수있도록 해야한다.
      if(btnNum >= 4){
        btnNum = 0;
      }
    ...
    */
    ...
    $(".playBtn").on("click",function(){
      ...
      //사이트 방문자가 재생버튼을 여러번 클릭하게 되면 auto1변수(스택메모리공간)에 있는 
      //setTimeout()메소드가 쌓여 있어 문제가 될 수 있음  setTimeout()메소드를 clearTimeout()메소드로 제거한 후
      // setTimeout()메소드 호출 구문의 실행을 해야한다.
      clearTimeout(auto1); // auto1변수에 저장된 setTimeout()메소드 제거
      auto1 = setTimeout(autoPlay, 1000); // 1초 지연 후 autoPlay함수 호출

      // 클릭한 재생 버튼 <img>태그의 src속성 주소값을 분홍색 이미지 주소로 변경해서 보여줌
      $("img",this).attr("src", "images/pop_btn_play_on.gif");
      // 클릭 안한 정지 버튼 <img>태그의 src속성 주소값을 회색 이미지 주소로 변경해서 보여줌
      $(".stopBtn img").attr("src", "images/pop_btn_stop_off.gif");
      
      return false;
    });

    // 정지 버튼 ■ <a>를 선택해서 가져와 click이벤트 등록
    $(".stopBtn").on("click",function(){

      clearTimeout(auto1); // auto1변수에 저장된 setTimeout()메소드 제거
      
      // 클릭한 정지 버튼 <img>태그의 src속성 주소값을 분홍색 이미지 주소로 변경해서 보여줌
      $("img",this).attr("src", "images/pop_btn_stop_on.gif");

      // 클릭 안한 재생 버튼 <img>태그의 src속성 주소값을 회색 이미지 주소로 변경해서 보여줌
 ...
 //-----------------------------------------------------------
  ...
  $("#tabmenu dt a").on("mouseover focus click", function(){
    ...
    // <a>룰 선택해서 <a>의 부모요소 <dt>선택하고 <dt> 다음에 작성된 <dd>태그 영역 최종선택
    // show()메소드로 보여줌
    $(this).parent().next().show();

    //3개의 이벤트(마우스오버, 포커스, 클릭)중 하나의 이벤트가 발생한 
    // <a>의 하위 <img>의 src속성의 주소값을 회색이미지에서 빨간색 이미지로 변경
    $("img",this).attr("src", 
                              $("img",this)
                              .attr("src")
                              .replace("out.gif","over.gif")
    ...
    //3개의 이벤트(마우스오버, 포커스, 클릭)중 하나의 이벤트가 발생한 <a>를 선택해 onTab변수에 다시 저장
    onTab = $(this);
