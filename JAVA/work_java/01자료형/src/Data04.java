...
//--------------------------------------------------------------------------------------------------------------
		...
		// 만일 128을 저장하려면 short 자료형으로 b 라는 이름의 변수 메모리를 선언해야한다.
		short b =128; // short 자료형 허용범위의 값( -32768 ~ 32767 사이의 값)만  변수에 저장 가능 
		
//		b = 32768;
		
		// 만일 32768 정수를 변수에 저장하려면 int 자료형으로 변수 메모리를 선언 해야한다.
		int c = 32768; // int 자료형 허용범위 값(-2의 31승 ~ 2의 31승 -1  사이의  값 )만 변수 c 메모리에 저장 가능 
								   //										 (	-2147483648	 ~  2147483648  )
		
		// 정수형 상수 중에서 접미사 L이 붙은 상수는 long 자료형(8byte 메모리 크기) 상수이므로 
		// int 자료형(4btye 메모리 크기) 변수 d 에 저장하면 역시 컴파일 에러가 발생한다.
//		int d = 123456L; 
		
		//그러므로 반드시 long 자료형으로 변수 d를 선언해야한다.
		long d = 123456L;
