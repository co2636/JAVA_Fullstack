/*
 	...
 	- 이 File 클래스는 실제로 파일에 데이터를 읽거나 쓰지는 않지만 파일 및 디렉터리 경로를 적용하여 파일 또는 디렉터리 정보를 얻을 수 있다.
 	...
 	- 주요 기능 및 메소드 
 		...
 		1. 파일 및 디렉터리 생성 
 			...
 			mkdir()  : 디렉터리 (폴더) 생성한다.
 			mkdirs() : 필요한 상위 디렉터리를 포함하여 여러 디렉터리를 생성한다.
 			
 		2. 파일 및 디렉터리 정보 확인 
 			exists() : 파일이나 디렉터리가 존재하는지 여부를 확인할 수 있다.
 			isFile() : 파일인지 확인한다.
 			isDirectory() : 디렉터리 인지 확인한다.
 			getName() : 파일 또는 디렉터리의 이름을 반환한다.
 		...
 		5. 파일 및 디렉터리 내부에 존재하는 목록 정보 
 			...
 			listFiles() : 디렉터리에 있는 파일 및 디렉터리를 File 객체 배열로 반환 
...
public class FileTest01 {
	public static void main(String[] args) throws IOException {
		...
		// 위 byteFileName byte 배열에 읽어온 파일이름을 String 문자열로 변환해서 저장할 변수 선언 
		String fileName = null;
		
		// 키보드에서 입력받은 파일이름을 InputStream 통로를 통해 
		// 위 byteFileName byte 배열의 크기 만큼 한번씩 읽어 들여 
		// 위 byteFileName byte 배열에 저장시킨다.
		// 요약 : 키보드에서 입력받은 값을 byteFileName 배열에 저장, read(byte[] b) 메소드를 호출해서 사용함 
		System.in.read(byteFileName);
		
		// 키보드에서 입력받은 파일이름 중 byteFileName 배열에 저장된 바이트 데이터들을 문자열로 변환시키면서 문자열객체 메모리에 저장한 후 
		// 양쪽 공백을 제거 후 반환 받아 저장 
		fileName = new String(byteFileName);
		
		// 입력한 파일명을 이용해 실제 만들어져있는 파일에 접근하기 위해 파일명을 포함한 파일경로명을 File 클래스의 객체 생성시 생성자로 전달해 저장 
		...
		// 파일을 읽기 모드로 열어 읽을 수 있느냐? 라고 물어보며 읽을수 있으면 true 반환, 없으면 false 반환 
		System.out.println("파일을 열어 읽을수 있는지에 대한 반환 값 : " + file.canRead());
		
		// 현재 파일이 쓰기모드로 열어 쓸수(기록) 있느냐? 물어보며 쓰기모드로 열 수 있으면 true 반환, 쓰기모드로 열수 없으면 false 반환 
